...
const currentConfig = reactive<StorageConfigApi.CreateStorageConfig & { _editId?: number }>({
  name: '',
  driver: 'minio',
  endpoint: '',
  accessKey: '',
...
// 获取连接信息显示文本
const getConnectionInfo = (record: StorageConfigApi.StorageConfig) => {
...
// 编辑
const handleEdit = (record: StorageConfigApi.StorageConfig) => {
...
// 删除
const handleDelete = async (record: StorageConfigApi.StorageConfig) => {
  try {
...
// 设为默认
const handleSetDefault = async (record: StorageConfigApi.StorageConfig) => {
  try {
  ...
  <div class="p-4">
    ...
    <div class="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700">
      ...
      <ul class="list-disc pl-5 space-y-0.5">
        <li><b>存储配置</b> = 一组存储连接信息（如 MinIO 的 endpoint + 密钥）</li>
        <li>可以为同一个驱动类型创建多个配置（如：生产环境 MinIO、测试环境 MinIO）</li>
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'driver'">
          <Tag :color="driverColors[record.driver] || '#8c8c8c'">
            {{ driverIcons[record.driver] || '📁' }} {{ driverLabels[record.driver] || record.driver }}
          </Tag>
        </template>
        <template v-else-if="column.key === 'connection'">
          <span class="text-sm text-gray-600">{{ getConnectionInfo(record) }}</span>
        </template>
    ...
    >
      <Form layout="vertical" class="mt-4">
        <Row :gutter="16">
          <Col :span="12">
            <Form.Item label="配置名称" required>
              <Input v-model:value="currentConfig.name" placeholder="如：生产环境 MinIO" />
            </Form.Item>
          </Col>
          <Col :span="12">
            <Form.Item label="存储驱动" required>
              <Select v-model:value="currentConfig.driver" placeholder="选择驱动" :disabled="isEditing">
        ...
        <template v-if="isExternalDriver">
          <template v-if="!isCOS">
            <Form.Item label="Endpoint" required>
              <Input v-model:value="currentConfig.endpoint" placeholder="如：minio.example.com:9000" />
            </Form.Item>
          </template>

          <template v-if="isCOS">
            <Form.Item label="Region" required>
              <Input v-model:value="currentConfig.region" placeholder="如：ap-guangzhou" />
            </Form.Item>
          </template>
          ...
          <Row :gutter="16">
            <Col :span="12">
              <Form.Item :label="isCOS ? 'Secret ID' : 'Access Key'" required>
                <Input v-model:value="currentConfig.accessKey" :placeholder="isCOS ? '腾讯云 Secret ID' : 'Access Key'" />
              </Form.Item>
            </Col>
            <Col :span="12">
          ...
          <Row :gutter="16">
            <Col :span="12">
              <Form.Item label="Bucket 名称" required>
                <Input v-model:value="currentConfig.bucket" placeholder="如：my-bucket" />
              </Form.Item>
            </Col>
            <Col :span="12">
              <Form.Item label="CDN 域名">
                <Input v-model:value="currentConfig.cdnDomain" placeholder="可选，如：cdn.example.com" />
              </Form.Item>
            </Col>
          </Row>
        ...
        <Form.Item label="描述">
          <Input.TextArea v-model:value="currentConfig.description" placeholder="备注信息" :rows="2" />
        </Form.Item>
      </Form>
      ...
      <template #footer>
        <Space>
          ...
          <Button v-if="isExternalDriver" :loading="testLoading" @click="handleTestConnection">
            测试连接
          </Button>
