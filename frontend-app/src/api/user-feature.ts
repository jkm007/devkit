...
export function kickAllOtherDevices() {
  return request.delete('/auth/devices/kick-all');
}
...
export function kickDevice(id: number) {
  return request.delete(`/auth/devices/${id}`);
}
...
export function getPrivacy() {
  return request.get<any>('/user/privacy');
}
...
export function updatePrivacy(params: {
  ...
  return request.put('/user/privacy', params);
}
...
export function getOAuthBindings() {
  return request.get<any[]>('/auth/oauth/bindings');
}
...
export function getOAuthBindUrl(provider: string) {
  return request.get<{ url: string }>('/auth/oauth/bind-url', { params: { provider } });
}
...
export function getSecurityLogs(params: { page?: number; pageSize?: number }) {
  return request.get<{ items: any[]; total: number }>('/auth/security-logs', { params });
}
...
export function getAvailableRoles() {
  return request.get<any[]>('/auth/role-applications/available-roles');
}
...
export function submitRoleApplication(params: {
  ...
  return request.post('/auth/role-applications', params);
}
...
export function getMyRoleApplications(params: { page?: number; pageSize?: number }) {
  return request.get<{ items: any[]; total: number }>('/auth/role-applications', { params });
}
