...
func Setup(ctx context.Context, cfg *config.Config, hub *ws.Hub) *gin.Engine {
	...
	authorized.Use(middleware.CaptchaGuard())
	{
		...
		system := authorized.Group("/system")
		{
			...
			system.GET("/questions/export", middleware.Permission("question:view"), questionHandler.Export)
			system.POST("/questions/batch-delete", middleware.Permission("question:delete"), questionHandler.BatchDelete)
			system.POST("/questions/batch-status", middleware.Permission("question:edit"), questionHandler.BatchStatus)
